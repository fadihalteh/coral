...
const populateDatabase = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const createdCategories = yield db.categories.bulkCreate(fakeCategories);
        // Insert fake brands into the database
        const createdBrands = yield db.brands.bulkCreate(fakeBrands);
        // Generate and insert fake data for Discounts
        const fakeDiscount = Array.from({ length: 20 }, () => generateFakeDiscount());
        const creartedDiscount = yield db.discounts.bulkCreate(fakeDiscount, { returning: true });
        // Generate and insert fake data for Users
        const fakeUsers = Array.from({ length: 50 }, () => generateFakeUser());
        const createdUsers = yield db.users.bulkCreate(fakeUsers, { returning: true });
        // Generate and insert fake data for Adresses
        const fakeAdresses = Array.from({ length: 100 }, () => generateFakeAddress(createdUsers));
        const createdAdresses = yield db.addresses.bulkCreate(fakeAdresses, { returning: true });
        ...
        const createdProducts = yield db.products.bulkCreate(fakeProducts, { returning: true });
        ...
        const createdReviews = yield db.reviews.bulkCreate(fakeReviews, { returning: true });
        ...
        const createdWishlist = yield db.wishlists.bulkCreate(fakeWishlist, { returning: true });
        ...
        const createdShoppingCarts = yield db.shoppingCarts.bulkCreate(fakeShoppingCart, { returning: true });
        ...
        const createdOrders = yield db.orders.bulkCreate(fakeOrders, { returning: true });
        ...
        const createdOrderItems = yield db.ordersItems.bulkCreate(fakeOrderItems, { returning: true });
        // Generate and insert fake data for Sessions
        const fakeSessions = Array.from({ length: 30 }, () => generateFakeSession(createdUsers));
        const createdSessions = yield db.sessions.bulkCreate(fakeSessions, { returning: true });
