...
const populateDatabase = async () => {
    try {
        const createdCategories = await index_1.default.categories.bulkCreate(fakeCategories);
        // Insert fake brands into the database
        const createdBrands = await index_1.default.brands.bulkCreate(fakeBrands);
        // Generate and insert fake data for Discounts
        const fakeDiscount = Array.from({ length: 20 }, () => generateFakeDiscount());
        const creartedDiscount = await index_1.default.discounts.bulkCreate(fakeDiscount, { returning: true });
        // Generate and insert fake data for Users
        const fakeUsers = Array.from({ length: 50 }, () => generateFakeUser());
        const createdUsers = await index_1.default.users.bulkCreate(fakeUsers, { returning: true });
        // Generate and insert fake data for Adresses
        const fakeAdresses = Array.from({ length: 100 }, () => generateFakeAddress(createdUsers));
        const createdAdresses = await index_1.default.addresses.bulkCreate(fakeAdresses, { returning: true });
        ...
        const createdProducts = await index_1.default.products.bulkCreate(fakeProducts, { returning: true });
        //Generate and insert fake data for Products Images 
        const fakeProductsImages = createdProducts.map((product) => ({
            image_url: faker.random.arrayElement(productImage),
            product_id: product.id,
        }));
        const createdProductsImages = await index_1.default.productsImages.bulkCreate(fakeProductsImages, { returning: true });
        ...
        const createdReviews = await index_1.default.reviews.bulkCreate(fakeReviews, { returning: true });
        ...
        const createdWishlist = await index_1.default.wishlists.bulkCreate(fakeWishlist, { returning: true });
        ...
        const createdShoppingCarts = await index_1.default.shoppingCarts.bulkCreate(fakeShoppingCart, { returning: true });
        ...
        const createdOrders = await index_1.default.orders.bulkCreate(fakeOrders, { returning: true });
        ...
        const createdOrderItems = await index_1.default.ordersItems.bulkCreate(fakeOrderItems, { returning: true });
        // Generate and insert fake data for Sessions
        const fakeSessions = Array.from({ length: 30 }, () => generateFakeSession(createdUsers));
        const createdSessions = await index_1.default.sessions.bulkCreate(fakeSessions, { returning: true });
        ...
        await index_1.default.sequelize.close();
    }
};
exports.populateDatabase = populateDatabase;
(0, exports.populateDatabase)();
