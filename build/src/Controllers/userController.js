...
const createUser = async (req, res) => {
    try {
        ...
        const newUser = await db.users.create({ username, email, password: hashedPassword, first_name, last_name, mobile, birth_date });
        const session_key = generateRandomString(30);
        const newSession = await db.sessions.create({ user_id: newUser.id, session_key });
