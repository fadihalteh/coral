...
const toggleProductInWishlist = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { error, value } = reviewSchema.validate(req.body);
        if (error) {
            return res.status(400).json({ error: error.details[0].message });
...
const removeProductFromWishlist = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const existingWishlist = yield db.wishlist.findOne({ where: { user_id: req.session.user_id, product_id } });
        if (!existingWishlist) {
            return res.status(404).json({ error: 'user does not have  this product in their wishlist' });
        }
        yield db.wishlist.destroy({ where: { user_id: req.session.user_id, product_id } });
...
const getUserWishlist = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const wishlists = yield db.wishlists.findAll({ where: { user_id: req.session.user_id } });
